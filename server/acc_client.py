    ...
    async def handle_raw_packet(self, data: bytes):
        ...
        if isinstance(packet, RegistrationResult):
            ...
            telemetry_dict = {
                ...
                "last_lap_ms": packet.last_lap.laptime_ms if packet.last_lap and packet.last_lap.laptime_ms else None,
                "session_time": 0.0,
                "track_pos": float(packet.spline_position),
                "coord_x": float(packet.world_pos_x),
                "coord_y": float(packet.world_pos_y),
                "coord_z": float(packet.world_pos_y),  # In ACC ist world_pos_y die zweite Ebene für 2D-Streckenkarten
                "yaw_rate": 0.0,
                "g_force_lat": 0.0,
                "g_force_lon": 0.0,
                "tyre_press_fl": 0.0,
